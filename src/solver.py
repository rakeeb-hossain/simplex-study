"""
BASE IMPLEMENTATION FORKED FROM: https://github.com/hasan-kamal/Linear-Program-Solvers/blob/master/solvers/simplex_solver.py
...
class SimplexSolver:
    ...
    def solve(self, c, A, b, pivot_rule):
        ...
        while not optimal:
            ...
            if iteration_number == 1:
                print('initial x_b = {}, with basic_indices = {}'.format(
                    x_b, basic_indices))
            """
            ...
            for j in indices:
                if j not in basic_indices:
                    ...
                    A_j = self.A[:, j]
                    reduced_costs[j] = self.c[j] - \
                        np.dot(c_b.T, np.dot(B_inv, A_j))
            ...
            self.pivot_time = self.pivot_time + elapsed
            self.num_pivots = self.num_pivots + 1
    ...
    def get_initial_bfs(self):
        ...
        if not contains_artifical:
            ...
            assert len(
                basic_indices) == M, 'assertion failed, please check this'
            assert matrix_rank(
                B) == M, 'this should have been equal, assertion failed'
